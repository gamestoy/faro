...
class ResourcesRequests {
  ...
  static _convertTrace(start, response, finish, initTime, url) {
    ...
    return {
      ...
      type: ResourcesRequests._convertContentType(response.args.data.mimeType, start.args.data.url),
  ...
  static _convertContentType(mimeType, url) {
    return !R.isNil(ContentType[mimeType])
      ? ContentType[mimeType]
      : ResourcesRequests._getContentTypeByFileExtension(url);
  }

  static _getContentTypeByFileExtension(url) {
