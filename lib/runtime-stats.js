...
const build = (events, before) => {
  ...
  const stats = S.map(initTime => {
    return parseStats(model, before, initTime);
  }, findNavigationStart(model));

  return S.fromMaybe({}, stats);
};

const parseStats = (model, before, initTime) => {
  const validEvents = getValidMainThreadEvents(model, before, initTime);
...
const getValidMainThreadEvents = (model, before, initTime) => {
  return S.filter(e => isValidEvent(e, before, initTime), model.mainThreadEvents());
};
