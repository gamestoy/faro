...
class ResourcesRequests {
  ...
  static _buildValidResourcesRequests(mainThreadEvents, before, it) {
    ...
    return S.map(t => {
      const resources = ResourcesRequests._buildResourcesRequests(t.resources, it);
  ...
  static _isResourceRequestEvent(event, before, initTime) {
    return (
      ...
      ((event.name === EventType.ResourceSendRequest && event.args.data.url) ||
        event.name === EventType.ResourceReceiveResponse ||
        event.name === EventType.ResourceFinish)
    );
  }

  static _mergeEvents(events, initTime) {
    const start = events[0];
    const response = events[1];
    const finish = events[events.length - 1];
    return ResourcesRequests._convertTrace(start, response, finish, initTime);
  }
  ...
  static _buildRequest(it) {
    return events => S.map(t => ResourcesRequests._mergeEvents(t, it), events);
  }

  static _filterIncompleteRequests(events) {
    return S.filter(t => t.length > 1, events);
  }
